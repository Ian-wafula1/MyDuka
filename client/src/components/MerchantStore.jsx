  ...
  return (
    <>
      ...
      <div className="card admins">
            ...
            .map((admin) => (
              <div key={admin.id} className="admin-item">
                ...
                <p>
                  ...
                  <span className={`status ${admin.account_status}`}>
                    {admin.account_status}
                  </span>
                </p>
                <div className="admin-actions">
                  <button
                    onClick={() => changeAdminStatus(admin)}
                    className={`btn ${
                      admin.account_status === 'active' ? 'btn-warning' : 'btn-success'
                    }`}
                  >
        ...
        <div>
          <button
            onClick={() => setIsOpen((prev) => ({ ...prev, admins: !prev.admins }))}
      ...
      <div className="card reports">
        ...
        <div className="reports-grid">
          ...
          <div className="report-item">
            ...
            <p>
              ...
              {store?.products?.filter((p) => p.quantity_in_stock > 0).length || 0}
            </p>
            <p>
              <strong>Low Stock Items:</strong>{' '}
              {store?.products?.filter((p) => p.quantity_in_stock < 10).length || 0}
            </p>
            <p>
          ...
          <div className="report-item">
            ...
            <p>
              ...
              {store?.users?.filter(
                (u) => u.account_type === 'admin' && u.account_status === 'active'
              ).length || 0}
            </p>
            <p>
              <strong>Recent Entries:</strong>{' '}
              {store?.entries?.filter((e) => e.payment_status === 'pending').length || 0}
            </p>
          </div>
        </div>
        ...
        <div className="report-actions">
          <button
            onClick={() => setIsOpen((prev) => ({ ...prev, reports: !prev.reports }))}
                ...
                {store?.entries?.slice(0, 5).map((entry) => (
                  <div key={entry.id} className="entry-item">
                    <p>
                      ...
                      {store.products?.find((p) => p.id === entry.product_id)?.name || 'Unknown'}
                    </p>
                    <p>
